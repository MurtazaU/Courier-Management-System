...
if (isset($_REQUEST['register-form-btn'])) {
  ...
  $query = $con->prepare('select * from customer where CustomerEmail = ?');
  $query->bindParam(1, $registeremail);
  $query->execute();
  $query_count = $query->rowCount();

  // If there is no account using the same email then proceed or else spit out an error
  if ($query_count == 0) {
    $sql = $con->prepare('insert into customer(CustomerName,CustomerEmail,CustomerPassword, CustomerZipCode, CustomerCountryId, CustomerState, CustomerCity, CustomerAddress, CustomerNumber, CustomerRegistrationDate) values (?,?,?,?,?,?,?,?,?,?)');
...
?>
    <script>
      alert('There is already another account using the same email')
    </script>
<?php
  }
}
    ...
    <div class="forms-container">
      <div class="signin-signup">
        ...
        <form action="" class="sign-in-form" method="POST" name="sign-in-form">
          ...
          <div class="input-field">
            <i class="fas fa-lock" aria-hidden="true" onclick="togglePassword()"></i>
            <input type="password" placeholder="Password" name="log-password" required />
          </div>
